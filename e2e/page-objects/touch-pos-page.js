...
class TouchPosPage extends BasePage {
  ...
  constructor(page) {
    ...
    // Grid of 5 payment buttons: mBoB, mPay, Cash, RTGS, Credit
    this.paymentMethodSection = page.getByText('Payment Method')
    this.mbobButton = page.getByRole('button', { name: /^mbob$/i })
    this.mpayButton = page.getByRole('button', { name: /^mpay$/i })
    this.cashButton = page.getByRole('button', { name: /^cash$/i })
    this.rtgsButton = page.getByRole('button', { name: /^rtgs$/i })
   ...
   * @param {'MBOB'|'MPAY'|'CASH'|'RTGS'|'CREDIT'} method
   */
  async selectPaymentMethod(method) {
    const buttonMap = {
      MBOB: this.mbobButton,
      MPAY: this.mpayButton,
      CASH: this.cashButton,
      RTGS: this.rtgsButton,
