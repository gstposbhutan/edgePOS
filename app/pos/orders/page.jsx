...
export default function OrdersPage() {
  ...
  const [section,  setSection]      = useState(() => searchParams.get('section') || 'SALES')
  ...
  async function loadSalesOrders() {
    ...
    let query = supabase
      .from('orders')
      .select('id, order_no, order_type, order_source, status, grand_total, gst_total, buyer_whatsapp, payment_method, created_at, sales_order_id')
      ...
      <div className="glassmorphism border-b border-border px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground">
            {section === 'SALES' ? `${salesOrders.length} ${salesTab === 'SO' ? 'sales orders' : 'invoices'}` : `${orders.length} orders`}
          </p>
        </div>
        <Button variant="ghost" size="icon-sm" onClick={section === 'SALES' ? loadSalesOrders : fetchOrders}>
        ...
        {section === 'SALES' && (
          salesLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground">
              ...
              {salesTab === 'SO' && <Button size="sm" onClick={() => router.push('/salesorder')}>Create First Sales Order</Button>}
            </div>
          ) : (
            <div className="divide-y divide-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                      ...
                      {salesTab === 'SI' && order.sales_order_id && (
                        <span className="text-primary text-[10px]">← SO</span>
                      )}
                    </div>
                  </div>
                  <div className="text-right shrink-0">
