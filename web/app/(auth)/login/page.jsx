...
import { MARKETPLACE_URL } from "@/lib/hosts"

function LoginForm() {
  const router = useRouter()
  const searchParams = useSearchParams()
  // Only same-app paths are honoured — a full URL in ?redirect= would be an open redirect.
  ...
  const custDestination = redirect || MARKETPLACE_URL
  ...
  async function handleStaffSubmit(e) {
    ...
    go(redirect || ROLE_HOME[role] || ROLE_HOME.RETAILER)
  }
  ...
  async function handleCustSignin(e) {
    e.preventDefault()
    setError(null); setLoading(true)
    const { error: err } = await signIn(custEmail.trim(), custPassword)
    setLoading(false)
  ...
  return (
    <div className="w-full max-w-sm mx-4">
      ...
      <p className="text-center text-xs text-muted-foreground mt-2">
        ...
        Need a business account? <a href="/contact" className="text-primary hover:underline underline-offset-4">Contact us</a>
      </p>
    </div>
  )
}
