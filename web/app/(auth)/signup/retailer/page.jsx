...
import { Eye, EyeOff, Loader2, Phone, Store } from 'lucide-react'
...
function SignupForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Session is set server-side as httpOnly cookie — redirect to POS
      if (data.success) {
        router.push('/pos')
      } else {
  ...
  return (
    <div className="w-full max-w-sm mx-4">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">Retailer Registration</p>
      </div>

      <Card className="glassmorphism">
        <CardHeader>
          <CardTitle className="text-lg font-serif">Create Retail Account</CardTitle>
          <CardDescription>Set up your retail shop and start selling</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Shop Details</p>
            </div>

            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">Shop Name</label>
              <Input
                placeholder="e.g. Karma General Store"
                value={businessName}
      ...
      <div className="text-center mt-6 space-y-2">
        <p className="text-xs text-muted-foreground">
          ...
          <a href="/login" className="text-primary hover:underline underline-offset-4">Sign in</a>
        </p>
        <p className="text-xs text-muted-foreground">
          Wholesaler?{' '}
