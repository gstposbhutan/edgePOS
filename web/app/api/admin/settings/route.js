...
import { PRESET_KEYS, DEFAULT_PRESET, isSafeColor } from '@/lib/theme/vendor-theme'
...
export async function GET(request) {
  ...
  const { data: entity, error } = await supabase
    .from('entities')
    .select('id, name, whatsapp_no, tpn_gstin, shop_slug, marketplace_bio, marketplace_logo_url, delivery_mode, shifts_enabled, theme_preset, theme_primary, nqrc_enabled, nqrc_merchant_name, nqrc_merchant_city, nqrc_account_id, nqrc_psp_guid, nqrc_mcc, nqrc_account_tag')
    .eq('id', entityId)
    .single()
...
export async function PATCH(request) {
  ...
  // Cashier shifts + cash-drawer reconciliation — OWNER-only vendor toggle (off by default).
  if (subRole === 'OWNER' && body.shifts_enabled !== undefined) allowed.shifts_enabled = !!body.shifts_enabled

  // POS colour theme — OWNER-only. Preset is validated against the known set; the custom brand
  // colour is re-sanitised here (defence in depth) before it can be injected into a <style>.
  if (subRole === 'OWNER' && body.theme_preset !== undefined) {
    allowed.theme_preset = PRESET_KEYS.includes(body.theme_preset) ? body.theme_preset : DEFAULT_PRESET
  }
  if (subRole === 'OWNER' && body.theme_primary !== undefined) {
    const c = typeof body.theme_primary === 'string' ? body.theme_primary.trim() : ''
    allowed.theme_primary = c && isSafeColor(c) ? c : null
  }
