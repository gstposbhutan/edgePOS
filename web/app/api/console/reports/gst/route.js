...
// GST report for the distributor/wholesaler consoles. Aggregates the caller's settled orders over a
// period into output tax (GST collected on sales where they're the seller), input tax / ITC (GST paid
// on purchases where they're the buyer), net GST payable, and a taxable-vs-exempt turnover split.
// OWNER/MANAGER, entity-scoped.
const VENDOR_ROLES = ['DISTRIBUTOR', 'WHOLESALER']
const SALE_TYPES = ['POS_SALE', 'WHOLESALE', 'SALES_INVOICE', 'MARKETPLACE']   // carry output GST
const PURCHASE_TYPES = ['WHOLESALE', 'SALES_INVOICE']                          // intra-platform buys carry input GST
...
export async function GET(request) {
  try {
    ...
    console.error('[console/reports/gst] GET error:', err)
