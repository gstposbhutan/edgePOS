/**
 * Guided-tour overlay helper — bakes instructional text into the Playwright screen recording.
 *
 * Two layers, both injected client-side (no video editor, no app rebuild):
 *   • title card  — a full-frame branded card that announces each flow (`titleCard`)
 *   • caption bar — a lower-third bar carrying each step's instruction  (`caption`)
 *
 * How it survives navigation: the page-side controller is registered with `addInitScript`, so it is
 * re-created on every document load, and the current caption is restored from sessionStorage — a
 * click that navigates won't blank the bar. The overlay is `pointer-events:none` end-to-end, so it
 * never intercepts a Playwright click.
 *
 * Usage (in a `tour` project spec):
 *   const { installTour, titleCard, caption, clearCaption, beat } = require('../lib/tour-overlay')
 *   await installTour(page)                 // BEFORE the first navigation
 *   await page.goto('/pos')
 *   await titleCard(page, { kicker: 'VENDOR · POS', title: 'Ring up a cash sale', sub: '…' })
 *   await caption(page, { step: 1, title: 'Open the register', text: '…' })
 */
...
#tour-cap {
  ...
  max-width: min(900px, 84vw); padding: 18px 26px 18px 20px;
  background: linear-gradient(180deg, rgba(15,23,42,.95), rgba(11,18,32,.95));
  border: 1px solid rgba(212,175,55,.28); border-left: 4px solid #D4AF37;
  border-radius: 16px; box-shadow: 0 20px 55px rgba(0,0,0,.55);
...
#tour-cap .cap-badge {
  ...
  color: #1a1206; font-weight: 800; font-size: 21px;
...
#tour-card {
  ...
  align-items: center; justify-content: center; text-align: center; padding: 8vh 10vw;
...
// Runs in the page (registered via addInitScript → re-runs on every navigation).
const PAGE_SRC = `
(function () {
  ...
  api.caption = function (step, title, text) {
    ...
    if (step === '' || step == null) { badge.classList.add('hide'); }
    else { badge.classList.remove('hide'); badge.textContent = step; }
...
async function installTour(page) {
  await page.addInitScript({ content: PAGE_SRC })
}

// Full-frame branded card announcing a flow. Shows, holds, fades out.
async function titleCard(page, { kicker = '', title = '', sub = '' } = {}, { hold = 2800 } = {}) {
  await page.evaluate(([k, t, s]) => { window.__tourBuild && window.__tourBuild(); window.__tour.card(k, t, s) }, [kicker, title, sub])
  await page.waitForTimeout(hold)
  await page.evaluate(() => window.__tour && window.__tour.hideCard())
  await page.waitForTimeout(550)
}

// Lower-third caption for one step. Stays up (persisting across navigation) until the next caption.
async function caption(page, { step = '', title = '', text = '' } = {}, hold = 2100) {
  await page.evaluate(([st, t, x]) => { window.__tourBuild && window.__tourBuild(); window.__tour.caption(st, t, x) }, [step === '' ? '' : String(step), title, text])
  await page.waitForTimeout(hold)
}
