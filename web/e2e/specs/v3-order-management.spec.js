...
test.describe('Order Management', () => {
  ...
  test.describe('Order Detail', () => {
    ...
    test('displays status timeline', async ({ page }) => {
      ...
      test.skip(!hasSection, 'Status History section not present')
  ...
  test.describe('Refund', () => {
    ...
    test('manager can approve a pending refund', async ({ page }) => {
      ...
      test.skip(!hasRefunds, 'Refunds section not visible')

      const approveButton = page.locator('button:has-text("Approve")')
      const hasApprove = await approveButton.isVisible({ timeout: 2000 }).catch(() => false)
      test.skip(!hasApprove, 'Approve button not visible')
