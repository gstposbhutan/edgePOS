...
export function useCart(entityId, createdBy) {
  ...
  const applyDiscount = useCallback(async (itemId, input) => {
    ...
    if (error) {
      // Fallback: try without discount_type/discount_value (migration 070 not applied yet)
      ...
      if (fallback) {
        setCarts(prev => prev.map((c, i) =>
          i === activeIndex
            ? { ...c, cart_items: c.cart_items.map(ci => ci.id === itemId ? { ...fallback, discount_type: discountType, discount_value: discountValue } : ci) }
            : c
        ))
      }
      return
