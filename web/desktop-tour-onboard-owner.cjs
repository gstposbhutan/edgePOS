// Desktop terminal ONBOARDING tour (shop OWNER), recorded via CHROMIUM against the running Electron
// app's served UI (:3200) + PocketBase (:8090). Sidesteps Playwright's broken Electron recordVideo
// (the "chromium-vs-:3200" workaround). Unlike the flow-only tours, this one EXPLAINS EVERY SCREEN:
// each key element is spotlit with callout() and captioned so a new owner learns what everything does.
// Reuses the WEB overlay helper (installTour re-runs on every navigation via addInitScript).
//
// Author-only: DO NOT run here. Intended runner (once the app is served at :3200 + PB at :8090):
//   node desktop-tour-onboard-owner.cjs
...
// One shared superuser session (admin@pos.local is a PB superuser in the E2E box). Returns the auth
// headers, or null if we can't authenticate.
...
;(async () => {
  ...
  // NQRC settings. All seeds are best-effort — a schema mismatch degrades to caption-only callouts.
  ...
  // ── Sign in on the terminal (owner) → lands on the POS register (app root). ──
  ...
  // ────────────────────────────────────────────────────────────────────────────────────────────
  await page.goto(`${APP}/stock.html`, { waitUntil: 'domcontentloaded' }); await beat(page, 1200)
  ...
  // ────────────────────────────────────────────────────────────────────────────────────────────
  await page.goto(`${APP}/adjustments.html`, { waitUntil: 'domcontentloaded' }); await beat(page, 1200)
  ...
  await page.getByText('MKT-TOUR-0001').first().waitFor({ timeout: 20000 }).catch(() => {})
  ...
  await page.getByText('WHS-TOUR-0001').first().waitFor({ timeout: 20000 }).catch(() => {})
  ...
  // ────────────────────────────────────────────────────────────────────────────────────────────
  await page.goto(`${APP}/settings.html`, { waitUntil: 'domcontentloaded' }); await beat(page, 1200)
  ...
  await browser.close()
  console.log('OWNER_TOUR_OK')
})().catch((e) => { console.error('OWNER_TOUR_FAIL', e.message); process.exit(1) })
