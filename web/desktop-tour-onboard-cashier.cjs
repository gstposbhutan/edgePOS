// Desktop terminal CASHIER onboarding tour, recorded via CHROMIUM against the running Electron app's
// served UI (:3200) + PocketBase (:8090). Sidesteps Playwright's broken Electron recordVideo. Reuses
// the web overlay helper. Ported from desktop/e2e/electron/tour-onboard-desktop-cashier.spec.ts —
// same seeds, same cashier login, same callouts/captions/title-cards in the same order.
const { chromium } = require('playwright')
const { installTour, titleCard, caption, clearCaption, clearHighlight, callout, beat } = require('./e2e/lib/tour-overlay')
...
// A dedicated cashier login, seeded into the `users` collection (role: cashier). The desktop login flow
// (pb.collection('users').authWithPassword) accepts any seeded user, so the terminal boots straight
// into the cashier's (role-gated) view.
const CASHIER = { email: 'cashier@pos.local', password: 'cashier12345', name: 'E2E Cashier' }

// One shared superuser session (admin@pos.local is a PB superuser on the E2E box). Returns auth headers
...
async function seedCashier(headers) {
...
// Sign in as the seeded cashier. Defensively clears any persisted session (a prior tour may have logged
// in as the owner), reloads to the login form, then signs in. All-tolerant — no brittle expect() gates.
...
;(async () => {
  ...
  // NQRC settings, and the cashier account exists to sign in with. All seeds are best-effort.
  ...
  await installTour(page) // BEFORE the first navigation — re-injects on every load via addInitScript
  ...
  // Build a cart so we can explain the customer/GST/checkout controls and the payment flow.
  ...
  await browser.close()
  console.log('CASHIER_TOUR_OK')
})().catch((e) => { console.error('CASHIER_TOUR_FAIL', e.message); process.exit(1) })
