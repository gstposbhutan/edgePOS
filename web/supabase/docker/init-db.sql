-- Supabase DB initialization script
-- Sets up roles, schemas, extensions, and the _supabase database

-- Ensure postgres role exists (Supabase image only pre-creates supabase_admin)
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'postgres') THEN
    CREATE USER postgres WITH SUPERUSER CREATEDB CREATEROLE REPLICATION BYPASSRLS PASSWORD 'postgres';
  END IF;
END $$;
