 ...
 * opened the search sheet and the rest raced that sheet's focus timer, so fast scans lost their
 * leading characters. The mirror of desktop/components/pos/keyboard/barcode-row.tsx.
 *
 * Navigation keys are deliberately NOT handled here — the page's listing listener treats this
 * field as an exception so ↑ ↓ still move the highlighted line while the caret stays put.
 *
 * @param {boolean}  disabled  A sheet is open — stop stealing focus until it closes.
 * @param {function} onSubmit  Enter with text in the field: a scanned code or a typed query.
 */
export function BarcodeRow({ disabled, onSubmit }) {
