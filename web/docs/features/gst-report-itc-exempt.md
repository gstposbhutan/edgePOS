...
4. **Desktop exempt ✅** — migration `022` + setup-pb (`gst_exempt` on `products` + `cart_items`); `lib/gst.ts` `calcItemTotals` per-line `gstExempt` (0 GST, total = taxable×qty) threaded through `calcCartTotals` (bill-discount branch keeps the flat-net approximation, same as web); `use-cart` stamps `gst_exempt` on the line + recomputes on qty/discount/price; `use-checkout` persists it on the order snapshot; product-form toggle + preview; bootstrap carries `gst_exempt` cloud→terminal (SELECT + PB insert in `electron/main.js`); sync-core maps it back onto `order_items` on ingest. `tsc` + `next build` green. *(Runtime-unverified on a real terminal — ships with the desktop release, like Phase 4.)*
5. **Retailer GST report ✅** — `/api/pos/reports/gst` + `/pos/reports` page + `GST Report` sidebar entry (MANAGER/OWNER/ADMIN — cashiers bounced). Mirrors the console report; sale types include `POS_SALE`/`MARKETPLACE`, purchase (ITC) side covers `WHOLESALE`/`SALES_INVOICE`/`PURCHASE_INVOICE` where the retailer is buyer. Same output/input/net + taxable-vs-exempt split.
