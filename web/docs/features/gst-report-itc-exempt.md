...
3. **Exempt in the B2B engine ✅** — `b2b-order.js` (`priceB2BCart` + SO→SI convert) routes through `lineGst`, reads `products.gst_exempt`, persists `order_items.gst_exempt`, sums line GST (exempt→0). Console product-form toggle + catalog GET/POST/PATCH + SellToBuyer estimate. **Remaining: the other web sales paths** — `web/hooks/use-cart.js` + `/api/pos/cart/items`, `/api/shop/{orders,checkout}` + `/api/cart/*`, `/api/sales/[id]/invoice`, `/api/wholesale/orders`, and their product forms (POS `product-form.jsx`/`entity-product-form`/`package-form`) — route through `web/lib/gst.js` + surface the toggle. Until then a retailer/marketplace sale of an exempt product still charges 5%.
