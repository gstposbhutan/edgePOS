...
- **Phase 4 — Desktop for the tiers:** 🟡 **web part DONE** — `TerminalsManager` console (`/{distributor,wholesaler}/terminals` + `Terminals` nav) reuses the entity-scoped `/api/cash-registers` routes: view back-office terminals, rename, re-download (rotate) the license key, deactivate. Licensing already provisions tier terminals in BACK_OFFICE (F3 + register-license binding). **Follow-on (own release cycle):** make the desktop Electron/PocketBase app tier-aware (a distributor/wholesaler BACK_OFFICE terminal showing incoming B2B orders + entity-level stock; per-warehouse stays web); needs PocketBase migrations + a desktop build/release, hard to verify in this env.
