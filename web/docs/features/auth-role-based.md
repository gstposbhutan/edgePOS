...
A single Supabase Auth identity that works across all three apps (`desktop`, `admin-hub`, `marketplace`). Access surfaces are determined by the user's role and sub-role. Row-Level Security enforces tenant isolation at the database layer — no application logic can bypass it.
