...
│   ├── /apps/admin-hub           # Admin SaaS
│   ├── /apps/marketplace         # Consumer portal
│   ├── /packages                 # Shared packages
│   ├── /services                 # WhatsApp gateway, sync worker, logistics bridge
│   └── /e2e                      # Playwright E2E tests
│
├── /desktop                      # Offline POS Terminal (Electron + PocketBase) [CURRENT DIRECTORY]
...
- ~~`parent_entity_id`~~: **Removed** — replaced by the `retailer_wholesalers` junction (a retailer can link to multiple wholesalers per category). See `web/docs/features/distributor-role.md`.
...
- `sold_by_weight`: Bool - Weighed/measured goods; PB `sale_price` is the per-unit (per-kg) rate and the cashier enters a weight at checkout (see `docs/label-maker-plan.md`)
...
**Status**: Offline retailer terminal shipped — licensing + activation + provisioning bootstrap, weighed goods + label maker, bundled pocketbase.exe + NSIS installer  
**Next Milestone**: production rollout — set cloud URLs, Authenticode-sign the installer, packaged-build QA (see `web/docs/pending-tasks.md`)